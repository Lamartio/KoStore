...
fun <K, V> mapInitializer(
        ...
        predicate: (K, V, Any) -> Boolean
): StoreInitializer<Map<K, V>>.() -> Unit = {

    addMiddleware { getState, dispatch, action, next ->
        middleware({ findValue(getState, predicate, action) }, dispatch, action, next)
    }

    addReducer { state: Map<K, V>, action ->
        val entry = state.entries.find { entry -> predicate(entry.key, entry.value, action) }
...
private fun <K, V> findValue(
        ...
        getState()
                .entries
                .find { entry -> predicate(entry.key, entry.value, action) }
                ?.value
