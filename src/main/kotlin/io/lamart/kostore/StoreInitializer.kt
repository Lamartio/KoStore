...
fun <T, R> StoreInitializer<T>.compose(map: (T) -> R, reduce: T.(R) -> T): StoreInitializer<R> = let { initializer ->
    object : StoreInitializer<R> {

        override fun addReducer(reducer: Reducer<R>) =
                reducer.compose(map, reduce).let(initializer::addReducer)

        override fun addMiddleware(middleware: Middleware<R>) =
                middleware.compose(map).let(initializer::addMiddleware)
